...
class Updater {
    ...
    processTarget() {
        ...
        if (this.pushController.getInstanceData().getWhereStatementController().has(key)) {
            ...
            if (updated_data.length !== new_updated_data.length) {
                checked = true;
            }
        ...
        if (old_data.length !== new_old_data.length) {
            checked = true;
        }
    ...
    checkRelationDataArray(object, statement, objects_to_update) {
        ...
        if (!object[statement.getRelation().getObjectName()].length) {
            if (!statement.getRelation().hasByObject(object)) {
                return false;
            }
        }
        let checked = false;
    ...
    checkRelationDataObject(object, statement, objects_to_update) {
        let new_model = null;
    ...
    hasWhereHasorDoesntHaveByKeys(statement) {
        if (!statement.hasWhereStatements()) {
            return false;
        }
        for (const key of this.collector.keys()) {
    ...
    hasWhereHasorDoesntHaveByKeysComplicated(statement) {
        if (!statement.hasWhereStatements()) {
            return false;
        }
        for (const key of this.collector.keys()) {
