import {WhereNoneHaveStatementCallbackStatement} from "./where-none-have-statement-callback-statement";
...
export class WhereNoneHaveStatementCallback {

    private statement: WhereNoneHaveStatementCallbackStatement;

    constructor(statement: WhereNoneHaveStatementCallbackStatement, callback: any) {
        this.statement = statement;
        callback(this);
    }

    public where(key: string | any , action?: string, value?: string|number): WhereNoneHaveStatementCallback {
        if (action) {
            this.statement.getWhereStatementController().add(new WhereStatement(key, action, value));
        } else {
            this.statement.getWhereStatementController().add(new WhereNoneHaveStatementCallbackStatement(key, this.statement.getRelation().getRelationObject()));
        }
        return this;
    }

    public orWhere(key: string | any , action?: string, value?: string|number): WhereNoneHaveStatementCallback {
        if (action) {
            this.statement.getWhereStatementController().addNewBag(new WhereStatement(key, action, value));
        } else {
            this.statement.getWhereStatementController().addNewBag(new WhereNoneHaveStatementCallbackStatement(key, this.statement.getRelation().getRelationObject()));
        }
        return this;
    }

    public whereBetween(key: string, low: number, high: number): WhereNoneHaveStatementCallback {
        this.statement.getWhereStatementController().add(new WhereBetweenStatement(key, low, high));
        return this;
    }

    public whereNotBetween(key: string, low: number, high: number): WhereNoneHaveStatementCallback {
        this.statement.getWhereStatementController().add(new WhereNotBetweenStatement(key, low, high));
        return this;
    }

    public whereIn(key: string, values: number[] | string[]): WhereNoneHaveStatementCallback {
        this.statement.getWhereStatementController().add(new WhereInStatement(key, values));
        return this;
    }

    public whereNotIn(key: string, values: number[] | string[]): WhereNoneHaveStatementCallback {
        this.statement.getWhereStatementController().add(new WhereNotInStatement(key, values));
        return this;
    }

    public whereHas(key: string, callback?: any): WhereNoneHaveStatementCallback {
    ...
    public whereDoesntHave(key: string, callback?: any): WhereNoneHaveStatementCallback {
        const relation = this.statement.getRelation().getRelationObject().getRelationContainer().findByObjectName(key);
    ...
    public whereNoneHave(key: string, callback?: any): WhereNoneHaveStatementCallback {
        const relation = this.statement.getRelation().getRelationObject().getRelationContainer().findByObjectName(key);
    ...
    public whereAllHave(key: string, callback?: any): WhereNoneHaveStatementCallback {
        const relation = this.statement.getRelation().getRelationObject().getRelationContainer().findByObjectName(key);
