import {OrderByStatement} from "../../../orderby/order-by-statement";
...
export class JoinCallback {
    ...
    public where(key: string | any , action?: string, value?: string|number): JoinCallback {
        if (action) {
            ...
            this.statement.getWhereStatementController().add(new WhereStatementCallbackStatement(key, this.statement.getRelation().getRelationObject()));
        }
        return this;
    }
    ...
    public orWhere(key: string | any , action?: string, value?: string|number): JoinCallback {
        if (action) {
            ...
            this.statement.getWhereStatementController().addNewBag(new WhereStatementCallbackStatement(key, this.statement.getRelation().getRelationObject()));
        }
        return this;
    }
    ...
    public with(name: string | string[], callback?: any): JoinCallback { //todo fix type
        if (callback && !Array.isArray(name)) {
            if (!this.statement.getRelation().getRelationObject().getRelationContainer().hasByObjectName(name)) { return this; }
            ...
            if (!Array.isArray(name)) {
                ...
                for (const v of name ) {
                    if (!this.statement.getRelation().getRelationObject().getRelationContainer().hasByObjectName(v)) { return this; }
    ...
    public orderBy(key: string, order?: string): JoinCallback {
